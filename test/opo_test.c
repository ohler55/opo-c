...
ojc_to_msg_test() {
  ...
  \"array\":[\n\
    ...
    opoVal		msg = opo_ojc_to_msg(&oerr, val, NULL);
...
msg_to_ojc_test() {
    ...
    ojcVal	val = opo_msg_to_ojc(&err, builder.head, NULL);
...
msg_to_ojc_dict_test() {
    ...
    ojcVal	val = opo_msg_to_ojc(&err, builder.head, dict);
...
bench_msg_to_ojc_test() {
	...
	if (NULL != (val = opo_msg_to_ojc(&err, builder.head, NULL))) {
	    ojc_destroy(val);
	} else {
...
bench_ojc_to_msg_test() {
  ...
  \"array\":[\n\
    ...
    for (int i = iter; 0 < i; i--) {
	if (NULL != (msg = opo_ojc_to_msg(&oerr, val, NULL))) {
	    free((uint8_t*)msg);
	} else {
	    ut_same_int(OPO_ERR_OK, err.code, "error transforming. %s", err.msg);
	}
...
append_opo_tests(utTest tests) {
    ...
    ut_appenda(tests, "opo.msg_to_ojc.dict", msg_to_ojc_dict_test, NULL);
