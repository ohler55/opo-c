...
get_test() {
    ...
    opoVal	top = opo_msg_top(builder.head);
    opoVal	v;
...
members_test() {
    ...
    *keys = '\0';
    for (; NULL != members; members = opo_val_next(members)) {
...
member_count_test() {
    ...
    ut_same_int(5, opo_val_member_count(&err, opo_msg_top(builder.head)), "count mismatch");
    opo_builder_cleanup(&builder);
}
